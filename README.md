...
# How to run: 
The python scripts run well in jupyter notebook, it is necessary to run the programs in the prespecified order to ensure that the stored csv files from the previous program is available for the next. 
...
3) Our strong assumption of historcial returns being representative for future returns requires us to have a minimum number of data for each asset. We specified this to be at least 3 years, therefore any asset with less than 3 years of trading history can not be used in our portfolio optimizer. 
